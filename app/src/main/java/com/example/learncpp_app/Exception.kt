...
class Exception : Fragment() {
    ...
    override fun onCreateView(
        ...
        vieww = inflater.inflate(R.layout.fragment_exception, container, false)
        ...
        btn2?.setOnClickListener {
            val nextFragment = Multithreading()
            val trans: FragmentTransaction = requireFragmentManager().beginTransaction()
            trans.replace(R.id.frame,nextFragment)
            (activity as? AppCompatActivity)?.supportActionBar?.title = getString(R.string.fourteen)
            trans.commit()
        }

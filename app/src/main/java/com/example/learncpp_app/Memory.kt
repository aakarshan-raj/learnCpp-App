...
class Memory : Fragment() {
    ...
    override fun onCreateView(
        ...
        vieww =  inflater.inflate(R.layout.fragment_memory, container, false)
        ...
        btn1.setOnClickListener {
            ...
            myEdit?.putBoolean("twelve", true)
            myEdit?.apply()

        }
        btn2?.setOnClickListener {
            val nextFragment = Exception()
            val trans: FragmentTransaction = requireFragmentManager().beginTransaction()
            trans.replace(R.id.frame,nextFragment)
            (activity as? AppCompatActivity)?.supportActionBar?.title = getString(R.string.thirteen)
            trans.commit()
        }

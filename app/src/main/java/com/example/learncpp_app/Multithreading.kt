...
class Multithreading : Fragment() {
    ...
    override fun onCreateView(
        ...
        vieww = inflater.inflate(R.layout.fragment_multithreading, container, false)
        ...
        btn1.setOnClickListener {
            ...
            myEdit?.putBoolean("fourteen", true)
            myEdit?.apply()

        }
        btn2?.setOnClickListener {
            val nextFragment = Debug()
            val trans: FragmentTransaction = requireFragmentManager().beginTransaction()
            trans.replace(R.id.frame,nextFragment)
            (activity as? AppCompatActivity)?.supportActionBar?.title = getString(R.string.fifteen)
            trans.commit()
        }

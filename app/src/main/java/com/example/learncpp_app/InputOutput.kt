...
class InputOutput : Fragment() {
    ...
    override fun onCreateView(
        ...
        vieww = inflater.inflate(R.layout.fragment_input_output, container, false)
        ...
        btn1.setOnClickListener {
            ...
            myEdit?.putBoolean("eleven", true)
            myEdit?.apply()

        }
        btn2?.setOnClickListener {
            val nextFragment = Memory()
            val trans: FragmentTransaction = requireFragmentManager().beginTransaction()
            trans.replace(R.id.frame,nextFragment)
            (activity as? AppCompatActivity)?.supportActionBar?.title = getString(R.string.twelve)
            trans.commit()
        }

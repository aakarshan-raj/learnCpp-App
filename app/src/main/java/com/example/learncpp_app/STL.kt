...
class STL : Fragment() {
    ...
    override fun onCreateView(
        ...
        vieww =  inflater.inflate(R.layout.fragment_s_t_l, container, false)
        ...
        btn2?.setOnClickListener {
            val nextFragment = InputOutput()
            val trans: FragmentTransaction = requireFragmentManager().beginTransaction()
            trans.replace(R.id.frame,nextFragment)
            (activity as? AppCompatActivity)?.supportActionBar?.title = getString(R.string.eleven)
            trans.commit()
        }
